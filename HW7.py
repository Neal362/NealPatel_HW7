...
def make_players_table(data, cur, conn):
    ...
    for player in data["squad"]:
        ...
        cur.execute("INSERT INTO Players(id, name, position_id, birthyear, nationality)VALUES(?,?,?,?,?)",(player["id"],player["name"],position_id, player["dateOfBirth"][:4],player["nationality"]))
    conn.commit()
## [TASK 2]: 10 points
...
def make_seasons_table(data, cur, conn):
    ...
    for i in data["seasons"]:
        if i["winner"]:
            cur.execute("SELECT id FROM Winners WHERE NAME = ?", (i["winner"]["name"], ))
            winner_id = int(cur.fetchone()[0])
            cur.execute("INSERT INTO Seasons(id, winner_id, end_year) VALUES(?,?,?)", (i["id"], winner_id, i["endDate"][:4]))
    conn.commit()

def winners_since_search(year, cur, conn):
    cur.execute("SELECT Seasons.id, Winners.name FROM Seasons JOIN Winners ON Seasons.winner_id = Winners.id WHERE Seasons.end_year >= ?", (year, ))
    dictionary = {}
    for i in cur.fetchall():
        dictionary[i[1]] = dictionary.get(i[1], 0) + 1
    return dictionary
...
class TestAllMethods(unittest.TestCase):
    ...
    def test_make_winners_table(self):
        ...
        self.assertEqual(winners_list[6][0], 6)
    ...
    def test_make_seasons_table(self):
        ...
        self.assertEqual(seasons_list[6][0], 258)


    def test_winners_since_search(self):

        result = winners_since_search(2010, self.cur2, self.conn2)
        self.assertEqual(len(result), 5)
        self.assertEqual(min(result.values()), 1)
